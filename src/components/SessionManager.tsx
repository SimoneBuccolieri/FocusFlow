...
import { usePomodoro, TimerMode } from '@/hooks/usePomodoro'; // Ensure TimerMode is exported from hooks
import { SessionModal } from './SessionModal';
import { Pencil, Play, Target, Sparkles, Coffee, Brain, BatteryCharging, Quote } from 'lucide-react';
...
export function SessionManager() {
    ...
    // Callback when timer finishes naturally
    const handleTimerComplete = useCallback((durationSeconds?: number) => {
        ...
        // Open modal
        setCompletedDuration(durationSeconds || 25 * 60);
        setIsModalOpen(true);
    }, []);

    const pomodoroState = usePomodoro(handleTimerComplete);
    const { mode, switchMode, isActive } = pomodoroState;
                                ...
                                <div className="space-y-6 flex-1 flex flex-col z-10">
                                    ...
                                    <div className="space-y-2 group flex-1 flex flex-col">
                                        ...
                                        <textarea
                                            ...
                                            className="w-full flex-1 bg-muted/50 dark:bg-white/5 border border-border/50 dark:border-white/5 rounded-2xl px-5 py-4 text-sm leading-relaxed placeholder:text-muted-foreground/30 focus:outline-none focus:ring-2 focus:ring-primary/50 focus:bg-background/50 dark:focus:bg-white/10 transition-all resize-none shadow-inner min-h-[120px]"
                                        />
                                    </div>
                                </div>
